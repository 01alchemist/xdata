...
export class AMFHelper {
    ...
    private initFlex():void {
        ...
        this.Flex.AbstractMessage.prototype = {
            ...
            readExternal: function (ba, parser) {
                let flagsArray = this.readFlags(ba);
                for (let i = 0; i < flagsArray.length; i++) {
                    let flags = flagsArray[i],
                        reservedPosition = 0;
                    ...
                    if ((flags >> reservedPosition) !== 0) {
                        for (let j = reservedPosition; j < 6; j++) {
        ...
        this.Flex.AsyncMessage = function () {
            ...
            //let correlationIdBytes; // byte array
        };
            ...
            for (let i = 0; i < flagsArray.length; i++) {
                ...
                if (i === 0) {
                    ...
                    if ((flags & this.CORRELATION_ID_BYTES_FLAG) !== 0) {
                        let correlationIdBytes = parser.readData(ba);
        ...
        this.Flex.SerializationProxy.prototype.readExternal = function (ba, parser) {
            /!*let saveObjectTable = null;
             let saveTraitsTable = null;
             let saveStringTable = null;
             let in3 = null;
    ...
    private readXMLDoc() {
        let index = this.readU29();
        if ((index & 1) === 0) return this.getObjectReference(index >> 1);

        //let xmldoc = new XML(this.readUTFBytes(index >> 1));
        let xmldoc = this.readUTFBytes(index >> 1);
    ...
    private readDate() {
        ...
        let d = new Date(this.readDouble());
    ...
    private readAmfObject() {
        ...
        if ((index & 3) === 1) {
            ...
            let isExternalizable:boolean = ((index & 4) === 4);
            let isDynamic:boolean = ((index & 8) === 8);
            let className:string = this.readString();

            let classMemberCount:number = (index >> 4);
            /!* uint29 *!/
            let classMembers:Array<string> = [];
            for (let i = 0; i < classMemberCount; ++i) {
        ...
        let aliasedClass = this.CLASS_ALIAS_REGISTRY[traits.type];
        ...
        if (traits.externalizable) {
            ...
            try {
                if (traits.type.indexOf("flex.") === 0) {
                    // Try to get a class
                    let classParts = traits.type.split(".");
                    let unqualifiedClassName = classParts[(classParts.length - 1)];
                    if (unqualifiedClassName && this.Flex[unqualifiedClassName]) {
                        let flexParser = new this.Flex[unqualifiedClassName]();
            ...
            let l = traits.members.length;
            let key;

            for (let j = 0; j < l; ++j) {
                let val = this.readAmfData();
    ...
    private readXML() {
        ...
        //let xml = new XML(this.readUTFBytes(index >> 1));
        let xml = this.readUTFBytes(index >> 1);
    ...
    private readByteArray() {
        ...
        let len = (index >> 1);
        //let ba2 = new DataArray();
        let ba2 = new DataArray(new ArrayBuffer(len));
        /!*while(len--) {
         let b = this.readUnsignedByte().toString(16).toUpperCase();
         if (b.length < 2) b = '0' + b;
         ba2.push('0x' + b);
         }*!/
