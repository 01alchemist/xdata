...
if (phantom.args.length === 0) {
    ...
    let pages = [], page, address, resultsKey, i, l;

    let setupPageFn = function(p, k) {
        return function() {
        ...
        let ival = setInterval(function(){
            if (isFinished()) {
                ...
                let fs = require("fs"),
                    ...
                    output;
                for (let filename in xml_results) {
                ...
                let specs = Number(results[1]);
                let failures = Number(results[2]);
